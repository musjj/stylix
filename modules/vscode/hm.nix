  ...
  config = lib.mkIf (config.stylix.enable && config.stylix.targets.vscode.enable) {
    programs.vscode = {
      ...
      userSettings = {
        ...
        "editor.fontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0);
        "debug.console.fontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0);
        "markdown.preview.fontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0);
        "terminal.integrated.fontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0);
        "chat.editor.fontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0);
        ...
        "editor.minimap.sectionHeaderFontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0 * 9.0 / 14.0);
        "scm.inputFontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0 * 13.0 / 14.0);
        "screencastMode.fontSize" = builtins.ceil (sizes.terminal * 4.0 / 3.0 * 56.0 / 14.0);
      };
    };
  };
}
